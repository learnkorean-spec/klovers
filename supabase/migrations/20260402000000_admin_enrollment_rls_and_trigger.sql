-- 1. Allow admins to INSERT enrollments for any user
...
-- 2. Allow admins to INSERT group members for any user
DROP POLICY IF EXISTS "Admins can insert group members" ON public.pkg_group_members;
CREATE POLICY "Admins can insert group members"
 ON public.pkg_group_members
 FOR INSERT
 WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));

-- 3. Update validate_enrollment trigger to skip strict checks for manual enrollments
