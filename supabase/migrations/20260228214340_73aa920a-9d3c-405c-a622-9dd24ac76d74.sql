...
-- Fix the validation trigger to use UPPERCASE status values matching existing data
...
BEGIN
  ...
  END IF;
  IF NEW.approval_status NOT IN ('PENDING','APPROVED','REJECTED','pending','approved','rejected') THEN
