...
const{ onGameComplete }: { onGameComplete?: (score: number, total: number) => void } KDramaQuizGame = () => {
  ...
  if (round >= totalRounds) {
    return (
      <section className="py-12 px-4"><Card className="max-w-lg mx-auto p-8 text-center space-y-4">
        ...
        <h2 className="text-2xl font-bold text-foreground">K-Drama Expert!</h2>
    ...
    <section className="py-12 px-4">
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <Card className="p-6 text-center space-y-4">
          <p className="text-sm text-muted-foreground">🎬 You hear this in a K-Drama:</p>
          <p className="text-4xl font-bold text-foreground">{q.korean}</p>
          <p className="text-xs text-muted-foreground">What does it mean?</p>
          ...
          {feedback && <p className={feedback === "correct" ? "text-green-600 dark:text-green-400 font-medium" : "text-destructive font-medium"}>
            {feedback === "correct" ? "✅ 대박! +5 XP" : `❌ Answer: ${q.answer}`}
          </p>}
        </Card>
