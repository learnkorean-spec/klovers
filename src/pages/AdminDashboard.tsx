                      ...
                      <TableBody>
                        {pagedUsers.map((u) => (
                          <TableRow key={u.user_id} className={cn("odd:bg-muted/30 hover:bg-muted/50 transition cursor-pointer", selectedStudentId === u.user_id && "ring-2 ring-primary/40")} onClick={() => setSelectedStudentId(selectedStudentId === u.user_id ? null : (u.enrollment_id ? u.user_id : null))}>
                            ...
                            <TableCell className="py-3 px-3 text-right font-mono">{u.amount_due > 0 ? <span className="text-destructive">{u.currency === "EGP" ? "LE" : "$"}{Math.round(u.amount_due).amount_due).amount_due).toLocaleString()}</span> : "—"}</TableCell>
                            <TableCell className="py-3 px-3">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {editingUnitPrice[e.id] !== undefined ? (
                                      ...
                                      <span className="texMath.round(e.unit_price)dium text-foreground">${e.unit_price}</span>
                                    )}
                                  </div>
