...
const SignUpPage = () => {
  ...
  const handleSocialLogin = async (provider: "google" | "apple") => {
    ...
    if (error) {
      toast({ title: t("auth.signUpFailed"), description: `Could not sign in with ${provider}.`, variant: "destructive" });
    }
  };
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            ...
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <Input type="password" placeholder={t("auth.passwordMin")} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} />
              <Button type="submit" className="w-full" disabled={loading}>
                {loading ? t("auth.signingUp") : t("auth.signUp")}
              </Button>
              <p className="text-sm text-center text-muted-foreground">
                {t("auth.alreadyHaveAccount")} <Link to={redirectTo ? `/login?redirect=${encodeURIComponent(redirectTo)}` : "/login"} className="text-foreground font-semibold underline">{t("auth.logIn")}</Link>
              </p>
            </form>
          </CardContent>
        </Card>
